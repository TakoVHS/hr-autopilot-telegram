...
**Решение:**
- ✅ **Используй модель `gpt-4o-mini`** — быстрее, дешевле, стабильнее
- OpenAI Platform → Assistants → выбери Assistant → Model → `gpt-4o-mini`
- Увеличь таймаут в коде (не рекомендуется, Telegram ждёт <30 сек)

### 2.1. После смены модели бот не отвечает
**Симптом:** "⚠️ Сейчас не получается ответить. Попробуйте позже."

**Причины:**
1. Выбрана несовместимая модель (старые модели не поддерживают Assistants API v2)
2. Баланс OpenAI исчерпан (новая модель дороже)
3. Rate limits на новую модель

**Решение:**
```
1. Проверь выбранную модель:
   https://platform.openai.com/assistants
   → Должно быть: gpt-4o или gpt-4o-mini
   
2. Проверь баланс:
   https://platform.openai.com/usage
   → Если $0 — пополни минимум $5
   
3. Рекомендуемая модель: gpt-4o-mini
   - Быстрая (2-5 сек ответа)
   - Дешёвая ($0.15 / 1M tokens)
   - Стабильная для Assistants API
```
